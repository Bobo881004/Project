                ...
                for(i in 0 until item.length()) {
                    ...
                    val jsonReq = JsonObjectRequest(Request.Method.GET, url, null,
                        Response.Listener {
                            ...
                            val cImgRId = resources.getIdentifier(JSONObject(idToName[cId].toString())["id"].toString().toLowerCase(), "drawable", activity?.packageName)
                            val date = SimpleDateFormat("yyyy-MM-dd", Locale.TAIWAN).format(Date(it["gameCreation"].toString().toLong()))
                            val time = SimpleDateFormat("mm:ss", Locale.TAIWAN).format(Date(it["gameDuration"].toString().toLong()*1000))
                            ...
                            info.add(ListView_save(cName, cImgRId, date, time, endGame, kda, cs, gold))
